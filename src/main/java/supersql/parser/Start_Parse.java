...
import supersql.db.CsvRegister;
...
public class Start_Parse {
	...
	private static boolean sparqlQuery = false; //add ayumi
...
//	public static String ssqlquery = new String();
	...
	public Start_Parse(int id) {
		parseSSBasket(this.getSSQLQuery(), id);
	}
	...
	public Start_Parse(StringBuffer querybuffer) {
		parseSSBasket(querybuffer.toString(), 10000);
	}
	...
	public static void setSparqlQuery(boolean sparqlQuery) {  //add ayumi
		Start_Parse.sparqlQuery = sparqlQuery;
	}
	...
	public static boolean isSparqlQuery() {  //add ayumi
		return sparqlQuery;
	}
	...
	//	for(int i=0; i<tableNames.length;i++) {
			...
			if(GlobalEnv.csvFlag) {                                       //add ayumi
				...
				//				System.err.println("CSV load IO error");
					...
					for(String key: GlobalEnv.csvTable.keySet()) {
						if(GlobalEnv.csvTable.get(key).contains("(")) {
							String s = GlobalEnv.csvTable.get(key).split("[\\s]*[(][\\s]*")[0];
							if(tableName.equals(s)) {
									try {
										CsvRegister.copytoDB(GlobalEnv.csvTable.get(key));
						//			} catch (SQLException e) {
						//				System.err.println("CSV load SQL error");
						//			} catch (IOException e){
						//				System.err.println("CSV load IO error");
							...
							if(tableName.equals(GlobalEnv.csvTable.get(key))) {
								try {
									CsvRegister.copytoDB(tableName);
						//		} catch (SQLException e) {
						//			System.err.println("CSV load SQL error");
		...
		//	whereInfo.setDBPediaWhereQuery(where_c.toString().trim());
			whereInfo.setSparqlWhereQuery(where_c.toString().trim());

		else if (Start_Parse.isSparqlQuery()) {
		//	whereInfo.setSparqlWhereQuery(where_c.toString().trim());  //add ayumi
		//	SparqlManager.GetResult(whereInfo.getSparqlWhereQuery());
		//	SparqlManager.PlusCSVCount();
		}
		else
	...
	private void parseSSBasket(String query, int id){
		if(query==null) return;

		query = query.trim();
		if(!query.toLowerCase().contains("ssbasket")){
			GlobalEnv.addErr("didn't find 'GENERATE'. please start with 'SSBASKET'.");
			Log.err("didn't find 'GENERATE'. please start with 'SSBASKET'.");
		}else {
			try {
				String a = query.substring(0, query.toLowerCase().indexOf("ssbasket"));
				String b = query.substring(query.toLowerCase().indexOf("ssbasket"), query.toLowerCase().indexOf("ssql"));
				String c = query.substring(query.toLowerCase().indexOf("ssql"));
				String c_1 = new String();
				String c_2 = new String();
				String c_3 = new String();

				if(c.toLowerCase().contains("recursive_sparql")){
					c_1 = c.substring(c.toLowerCase().indexOf("generate"), c.toLowerCase().indexOf("recursive_sparql"));
					c_1 = c_1.substring(0, c_1.lastIndexOf("}"));
					if(c.toLowerCase().contains("recursive_expression")) {
						c_2 = c.substring(c.toLowerCase().indexOf("recursive_sparql")+17, c.toLowerCase().indexOf("recursive_expression")-1);
						c_2 = c_2.substring(0, c_2.lastIndexOf("}"));
						c_3 = c.substring(c.toLowerCase().indexOf("recursive_expression")+21, c.lastIndexOf("}"));
						c_3 = c_3.substring(0, c_3.lastIndexOf("}"));
					}else {
						c_2 = c.substring(c.toLowerCase().indexOf("recursive_sparql")+17);
						c_2 = c_2.substring(0, c_2.lastIndexOf("}"));
					}
				}else {
					if(c.toLowerCase().contains("recursive_expression")) {
						c_1 = c.substring(c.toLowerCase().indexOf("generate"), c.toLowerCase().indexOf("recursive_expression"));
						c_1 = c_1.substring(0, c_1.lastIndexOf("}"));
						c_3 = c.substring(c.toLowerCase().indexOf("recursive_expression")+21, c.lastIndexOf("}"));
						c_3 = c_3.substring(0, c_3.lastIndexOf("}"));
					}else {
						c_1 = c.substring(c.toLowerCase().indexOf("generate"), c.lastIndexOf("}"));
						c_1 = c_1.substring(0, c_1.lastIndexOf("}"));
					}
				}
...
//				Log.info(a);
		...
		//			if(!query.toLowerCase().contains("sparql")){
					while(true){
